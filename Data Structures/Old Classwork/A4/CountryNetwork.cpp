...
bool CountryNetwork::detectLoop() {
    ...
    if (ptr->name == head->name) return 1;
    else return 0;
}
...
void CountryNetwork:: readjustNetwork(int start_index, int end_index)
{
    ...
    else if (end_index < start_index) printf("Invalid indices\n");
    else {
        Country * sto = head;
        Country * temp = sto;
        for (int i = 0; i < start_index && sto != NULL; i++) {
            temp = sto;
            sto = sto -> next;
        }
        Country * ptr = head;
        Country * ind = ptr;
        for (int i = 0; i < end_index && ptr != NULL; i++) {
            ind = ptr;
            ptr = ptr->next;
        }
        temp->next = ptr;
